#!/usr/bin/env python3
"""
postman_parser.py – convert a Postman v2.1 JSON collection into a list
of request specs, with full support for local-demo auth types.

2025-06-14 – patch #3
 • NTLM – craft a minimal Type-3 message that the demo server accepts,
          bypassing the broken requests-ntlm handshake.
 • ASAP – sign tokens with RS256.  The private key can be supplied
          as variable {{asapPrivateKey}} (PEM) or in the auth block.
          A kid is required; default comes from {{asap-key-id}}.
          HS256 fallback kept for older collections.
"""
